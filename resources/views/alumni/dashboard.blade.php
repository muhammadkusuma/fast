...
@section('sidebar-menu')
    ...
    {{-- Logika Menu: Jika sudah isi, link bisa diarahkan ke halaman detail/edit, jika belum ke create --}}
    <a href="{{ $sudahIsiTracer ? '#' : route('alumni.tracer.create') }}" class="flex items-center px-4 py-2 text-gray-600 hover:bg-blue-50 hover:text-primary rounded-lg transition-colors">
        ...
        {{ $sudahIsiTracer ? 'Lihat Data Tracer' : 'Isi Tracer Study' }}
    </a>
...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden group">
            <div class="absolute right-0 top-0 mt-4 mr-4 w-12 h-12 {{ $sudahIsiTracer ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500' }} rounded-full flex items-center justify-center group-hover:scale-110 transition-transform">
                @if($sudahIsiTracer)
            ...
            <div class="mt-4">
                @if(!$sudahIsiTracer)
                    ...
                    <a href="{{ route('alumni.tracer.create') }}" class="text-sm font-semibold text-red-500 hover:text-red-700 flex items-center">
                ...
                @else
                    <span class="text-sm font-semibold text-green-500 flex items-center">
                        Terima Kasih <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </span>
                @endif
            </div>
        </div>
        ...
        <div class="p-6">
            <ol class="relative border-l border-gray-200 ml-3">                  
                <li class="mb-10 ml-6">             
                    ...
                    <time class="block mb-2 text-sm font-normal leading-none text-gray-400">Tahun {{ date('Y') }}</time>
                    <p class="mb-4 text-base font-normal text-gray-500">Seluruh alumni angkatan {{ date('Y') - 1 }} wajib mengisi kuesioner tracer study sebagai syarat legalisir ijazah.</p>
                </li>
                <li class="mb-10 ml-6">
                    ...
                    <time class="block mb-2 text-sm font-normal leading-none text-gray-400">Desember {{ date('Y') }}</time>
                </li>
            </ol>
        </div>
    </div>
